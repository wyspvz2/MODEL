...
\[
L_\text{AT} = \frac{1}{B} \sum_{i=1}^{B} \| A(F_i^s) - A(F_i^t) \|_2^2
\]
...
class ATLoss(nn.Module):
    ...
    def attention_map(self, feature: torch.Tensor) -> torch.Tensor:
        ...
        feature: [B, C, H, W]
        returns: [B, H, W] normalized attention map
        """
        # 1. channel-wise squared sum
        attn = feature.pow(2).sum(dim=1)  # [B, H, W]
        # 2. flatten & normalize L2
        attn_flat = attn.view(attn.size(0), -1)
        attn_norm = attn_flat / (attn_flat.norm(p=2, dim=1, keepdim=True) + self.eps)
        attn_norm = attn_norm.view_as(attn)
        return attn_norm
    ...
    def forward(self, F_s: torch.Tensor, F_t: torch.Tensor) -> torch.Tensor:
        ...
        F_t: 教师特征 [B, C, H, W]
        """
        A_s = self.attention_map(F_s)
        A_t = self.attention_map(F_t)
...
if __name__ == "__main__":
    B, C, H, W = 4, 16, 8, 8  # batch, channels, height, width
    F_t = torch.randn(B, C, H, W)  # 教师中间特征
    F_s = torch.randn(B, C, H, W)  # 学生中间特征

    at_loss_fn = ATLoss()
    loss = at_loss_fn(F_s, F_t)
